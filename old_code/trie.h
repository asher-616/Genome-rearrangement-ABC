...
public:
	...
	Trie();
	Trie(int);
	//Trie(const Trie&); //copy constructor. need to write later
	~Trie();
	int key;
	//bool isLeaf; //I (probably) dont need this since I have flag to mark end of sequences and leaf can be identify since sons vector size should be 0
	bool flag; //this is used to mark nodes that represent a block. when adding a seq to the trie, only the last node in the seq is flagged
	vector<Trie> sons;

	void insert(vector<int>&);
	//bool deletion(Trie*&, std::string); //don't need for now
	//bool search(std::string); //don't need for now
	//bool haveChildren(Trie const*); //don't need for now
	
private:
	int findOrCreateSon(int);// look for the son and returns its index (if doesn't exist, it creates it)

};



//djhaskdghdksfjghdksfghxdlskfughsliushgfkljisdhflkgjdhsfklghsdflkghsfklhsbfkljnckjbgd
/*
#include <iostream>
