...
/*
void print_genome(vector<GenomeClass>) {
...
void genomes::init(vector<genomeType> & genomeVect) {
	for (vector<genomeType>::iterator it = genomeVect.begin(); it != genomeVect.end(); ++it)
	{
...
genomes::genomes(vector<genomeType> &genomeVect){
...
genomes::genomes(string inputFile) {
...
void genomes::calcUniqueBlocksBasedOnTree() { //not finished
	tree t(GRABC_options::_inputTreeFileName);
...
comparison between two genomes is done using the previously used compareBlocksGenomes method*/
	if (t->isLeaf()) {
		// nothing to do. just return genome to parent
